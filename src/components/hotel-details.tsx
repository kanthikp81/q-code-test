    ...
    <div className="flex flex-col text-left justify-start">
      <div>
        <div className="flex flex-row items-center mt-0">
          <h2 className="text-xl leading-6 font-bold mb-0 mt-0 overflow-hidden">
            {name.length > 30 ? name.substring(0, 30) + "..." : name}
          </h2>
          <Ratings rating={+rating.ratingValue} type={rating.ratingType} />
        </div>
        <p className="text-sm leading-6 mt-0">{address.join(", ")}</p>
      </div>

      <p className="text-sm leading-6 text-red-700 underline mt-0">
        {offerName}
      </p>
      <p className="mt-auto text-sm leading-6 leading-2 text-green-900">
